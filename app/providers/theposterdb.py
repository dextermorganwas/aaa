...
class ThePosterDBProvider:
    ...
    def _build_search_urls(self, title: str, lookup: Lookup, year: int | None) -> list[str]:
        ...
        # Start with the normal title search. This is the most compatible public TPDb search
        # endpoint and is also what the uploaded TPDb tools use. Identifier searches follow it
        # as additional evidence rather than replacing the title search outright.
        if title:
            add({'section': section, 'term': title})

        # Try the known IDs as ordinary search terms. TPDb has historically indexed these in
        # different ways, so this is useful when the title is translated, abbreviated, or shared.
        for value in (lookup.tmdb_id, lookup.imdb_id, lookup.tvdb_id):
            if value:
                add({'section': section, 'term': str(value)})

        # Newer TPDb search pages expose dedicated ID fields. Try them too, but only after the
        # broadly compatible searches above.
        for key, value in (
        ...
        # A title+year search is useful for disambiguating same-name releases, but should not
        # prevent the normal title lookup when TPDb does not recognize the year query parameter.
        if title and year:
            add({'section': section, 'term': title, 'year': str(year)})
